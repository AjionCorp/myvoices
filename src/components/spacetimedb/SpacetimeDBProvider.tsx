  ...
  if (db.conversation) {
    db.conversation.onInsert((_ctx: unknown, row: unknown) => {
      ...
      useMessagesStore.getState().addConversation({
        ...
        status: convo.status,
        requestRecipient: convo.requestRecipient ?? "",
        createdAt: Number(convo.createdAt),
        updatedAt: Number(convo.updatedAt),
      });
    });
    ...
    db.conversation.onDelete((_ctx: unknown, row: unknown) => {
      const convo = row as { id: unknown };
      useMessagesStore.getState().removeConversation(Number(convo.id));
    });
  }
